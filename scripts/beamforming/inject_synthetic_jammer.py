...
def _resolve_sample_rate(input_dir: Path, sample_rate: float | None) -> float:
    if sample_rate is not None:
        return sample_rate
    manifest = input_dir / "capture_metadata.json"
    if manifest.exists():
        meta = json.loads(manifest.read_text())
        if "sample_rate" in meta:
            return float(meta["sample_rate"])
    raise ValueError("sample rate required: pass --sample-rate or provide capture_metadata.json with sample_rate")
...
def main() -> None:
    ...
    print(json.dumps(inject_jammer(a.input_dir, a.output_dir, sample_rate, channels=a.channels, pattern=a.pattern, max_samples=a.max_samples, force=a.force), indent=2))
